...
main(VOID)
{
	...
	TstCond (Failure, (Three == Two + One) ,
   "3 != 2+1");
		TstCond (Failure, (Four == Three + One),
		   "4 != 3+1");
		printf(" %.6f ", 4.0 + 2.0* (-2.0));
			TstCond (Failure, (Four + Two * (- Two) == Zero),
		   "4+2*(-2) != 0");

		TstCond (Failure, (Four - Three - One == Zero),
		   "4-3-1 != 0");
...
*/
	...
	U1 = One / W;
	U2 = Radix * U1;
	printf("Closest relative separation found is U1 = %.7e, Precision = %f .\n\n", U1, Precision);
